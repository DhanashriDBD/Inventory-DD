...
csv_ob=open("EC2-Inventory.csv","wb",newline='')
...
for each_reg in list_regions:
    ...
    for each_item in response:
        for instances in each_item['Instances']:
            if instances['State']['Name']== 'running' or instances['State']['Name']== 'stopped':
                ipaddress = instances.get('PublicIpAddress')
                if ipaddress is None:
                    ipaddress = 'NA'
                    csv_w.writerow([instances['InstanceId'],instances['Placement']['AvailabilityZone'],
                    instances['InstanceType'],instances['State']['Name'],instances['PrivateIpAddress'],ipaddress,
